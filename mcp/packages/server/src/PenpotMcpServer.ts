...
import { DesignGenerateTool } from "./tools/design/DesignGenerateTool";
...
export class PenpotMcpServer {
	...
	private readonly transports = {
		...
		sse: {} as Record<
			string,
			{ transport: SSEServerTransport; userToken?: string }
		>,
	};
	...
	constructor(private isMultiUser: boolean = false) {
		...
		this.webSocketPort = parseInt(
			process.env.PENPOT_MCP_WEBSOCKET_PORT ?? "4402",
			10,
		);
	...
	public isRemoteMode(): boolean {
		const isRemoteModeRequested: boolean =
			process.env.PENPOT_MCP_REMOTE_MODE === "true";
	...
	public getInitialInstructions(): string {
		...
		instructions = instructions.replace(
			"$api_types",
			this.apiDocs.getTypeNames().join(", "),
		);
		return instructions;
	}
	...
	private registerTools(): void {
		...
		toolInstances.push(new DesignGenerateTool(this));
	...
	async start(): Promise<void> {
		...
		return new Promise((resolve) => {
			this.app.listen(this.port, this.host, async () => {
				...
				this.logger.info(
					`Modern Streamable HTTP endpoint: http://${this.host}:${this.port}/mcp`,
				);
				this.logger.info(
					`Legacy SSE endpoint: http://${this.host}:${this.port}/sse`,
				);
				this.logger.info(
					`WebSocket server URL: ws://${this.host}:${this.webSocketPort}`,
				);
