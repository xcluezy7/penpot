# Requirements: PinPot — Multiplayer Agent-First Design Tool
...
5. **Primary agent compatibility** — OpenClaw and pi coding agent as first-party integrations
6. **Docker deployment** — single unified container with all services (frontend, backend, MCP, agent bridge)
7. **Codex/Claude compatibility** — secondary, in a later release
...
4. **Docker-friendly**: The bridge runs as an additional container in the existing Docker Compose setup.
5. **Agent-first by design**: The bridge is built from the ground up for agent interaction, not bolted onto an existing API.
...
3. Design skills (prototype generation, animation, variant exploration) work within Penpot's native format (not HTML output)
